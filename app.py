...
def update_assignment(id):
    ...
    try:
        ...
        assignment.student_id = data['student_id']  # Ensure you update the student_id if needed
        db.session.commit()
...
def report_assignments_per_student():
    # Using prepared statements to get the assignments for each student
    query = text("""
        SELECT 
            s.id AS student_id,
    ...
    for row in result:
        ...
        if row.assignment_title:  # Only add assignments if they exist
            # Ensure due_date is treated as a date
            ...
            report_data[student_id]['assignments'].append({
                ...
                'due_date': due_date  # Store as a date object
            })
